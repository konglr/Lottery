...
def load_historical_data(analysis_period):
    try:
        ...
        st.error("找不到 CSV 文件 'data/双色球_processed_data.csv'，请运行数据导入脚本。")
        return pd.DataFrame()
    ...
    if st.button("开始 AI 预测"):
        if not gemini_api_key:
            ...
            try:
                ...
                # Get last 20 records
                recent_data = load_historical_data(20)
...
以下是最近20期的开奖结果（按时间倒序排列）：
{records_str}

请基于以上数据进行深度分析：
1. 分析红球的走势（冷热号、和值趋势、奇偶比、大小比等）。
2. 分析蓝球的走势。
3. 给出10组预测的号码（6个红球 + 1个蓝球）。红球范围1-33，不能重复；蓝球范围1-16。并说明每组号码的预测理由。

请严格按以下格式输出结果：
### 📊 数据分析理由
[在此详细说明你的分析依据，包括趋势、概率、冷热号、遗漏值等]
