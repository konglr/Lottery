...
- [多模型预测引擎 (Models A-J)](../prediction_models/SKILL.md) - 包含统计、机器学习、深度学习及启发式算法的综合预测体系。
...
- `request_data_update.py`: **增量与覆盖更新脚本**。支持 `>=` 期号逻辑，确保中奖细节完整刷新。
- `request_process_all_data.py`: **数据清洗与特征生成脚本**。支持奖级归一化（如福运奖）与自动列清理。
- `skills/`: 项目知识库。
    - [数据管理与处理规则](../data_management/SKILL.md) - **核心文档**：记录了增量覆盖逻辑、奖项映射及字段清理规则。
    - [各彩种详细规则](./SKILL.md) - 包含双色球、大乐透等游戏的官方中奖标准。
