...
def get_prediction_prompt(history_text, config):
    ...
    prompt += f"4. **胆码推荐**：请从你选出的{ '号码' if is_kl8 else '红球' }中，选出 1-4 个最看好的作为“胆码”。\n"
    ...
    if is_kl8:
        ...
        prompt += "... (以此类推完成 5 组)\n"
        if config['code'] == 'qlc':
            prompt += "注：七乐彩的基本号和特别号共用 1-30 的号码池，且互不重复。这意味着你实际上是在 30 个号码中选取 7 个不重复的号码（6个红球+1个蓝球）。请在分析时将这 7 个位置作为一个整体进行考量，以提高全局命中概率。\n"
        prompt += "【预测结果结束】\n"
